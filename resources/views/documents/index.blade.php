...
@section('title', 'Data Arsip - SI-ARSIP DPMPTSP')

@section('header-title', 'Sistem Informasi Arsip Surat Keputusan')
@section('header-subtitle', 'Data Arsip')
...
<div class="content-card">
    <div class="search-section">
        ...
        <div style="display: flex; gap: 12px; align-items: center;">
            <form method="GET" action="{{ route('documents.index') }}" style="display: flex; gap: 12px; align-items: center; position:relative;">
            ...
            <div class="filter-dropdown">
                ...
                <div id="filterDropdown" class="filter-dropdown-content" style="min-width:280px;">
                    <form id="filterForm" method="GET" action="{{ route('documents.index') }}" style="padding:8px 0;">
                        ...
                        @foreach(($categories ?? []) as $cat)
                            ...
                            <a href="#" class="filter-option" onclick="setCategory('{{ $slug }}', {{ $cat->id }})">
                        ...
                        <input type="hidden" name="category_id" id="catIdInput" value="{{ request('category_id','') }}">
                        ...
                        <input type="hidden" name="per" value="{{ request('per', $documents->perPage()) }}">
            ...
            <div class="filter-dropdown">
                ...
                <div id="sortDropdown" class="filter-dropdown-content">
                    ...
                    <a href="{{ route('documents.index', array_merge(request()->query(), ['sort'=>'recent'])) }}" class="filter-option {{ $sort==='recent'?'active':'' }}">Terbaru</a>
                    <a href="{{ route('documents.index', array_merge(request()->query(), ['sort'=>'oldest'])) }}" class="filter-option {{ $sort==='oldest'?'active':'' }}">Terlama</a>
                    <a href="{{ route('documents.index', array_merge(request()->query(), ['sort'=>'popular'])) }}" class="filter-option {{ $sort==='popular'?'active':'' }}">Paling Populer</a>
                    <a href="{{ route('documents.index', array_merge(request()->query(), ['sort'=>'title_asc'])) }}" class="filter-option {{ $sort==='title_asc'?'active':'' }}">Judul A-Z</a>
                    <a href="{{ route('documents.index', array_merge(request()->query(), ['sort'=>'title_desc'])) }}" class="filter-option {{ $sort==='title_desc'?'active':'' }}">Judul Z-A</a>
                </div>
            </div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; font-size: 14px; color: #6c757d; gap:12px; flex-wrap:wrap;">
        <div>
            ...
            <select onchange="applyPer(this.value)" style="border: 1px solid #e9ecef; border-radius: 6px; padding: 6px 10px; margin: 0 8px;">
                @php $per = (int) request('per', $documents->perPage()); @endphp
                @foreach([10,25,50,100,250,500] as $opt)
        ...
        <table class="custom-table">
            ...
            <tbody>
                @if($documents->count() > 0)
                    ...
                    <tr>
                        <td>{{ str_pad($document->id, 3, '0', STR_PAD_LEFT) }}.16.74.1 / S / {{ $document->id }} / {{ date('Y') }}</td>
                        <td>
                            <div class="document-title">{{ $document->title }}</div>
                            <div class="document-meta">{{ $document->uploader->name ?? 'Unknown' }}</div>
                        </td>
                        <td>{{ optional($document->uploaded_at ?? $document->created_at)->format('d M Y') }}</td>
                        <td>{{ 'DPMPTSP KOTA' }}</td>
                        <td>
                            <span class="category-badge" style="background-color: {{ optional($document->documentType->subCategory->category)->color ?? '#365F8D' }};">
                                {{ optional($document->documentType->subCategory->category)->name ?? 'Umum' }}
                            </span>
                        </td>
                        <td>{{ Str::limit($document->documentType->name ?? $document->title, 50) }}</td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('documents.show', $document) }}" class="btn-action btn-view">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('documents.download', $document) }}" class="btn-action btn-download">
                                    <i class="fas fa-download"></i>
                                </a>
                                @if(Auth::user()->id === ($document->uploaded_by ?? null) || Auth::user()->is_admin)
                                <a href="{{ route('documents.edit', $document) }}" class="btn-action btn-edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form method="POST" action="{{ route('documents.destroy', $document) }}" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus dokumen ini?')">
                                    @csrf
                                    @method('DELETE')
