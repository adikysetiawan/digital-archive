...
@section('title', 'Dashboard - SI-ARSIP DPMPTSP')

@section('header-title', 'Sistem Informasi Arsip Surat Keputusan')
@section('header-subtitle', 'Overview & Statistik')
...
<p class="dashboard-subtitle" style="font-size: 16px; color: #6c757d; margin-bottom: 30px;">Selamat datang, {{ Auth::user()->name }}! Berikut adalah ringkasan sistem arsip digital.</p>
...
<div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 24px; margin-bottom: 30px;">
    <div class="stat-card" style="background: linear-gradient(135deg, #365F8D 0%, #2A4A6B 100%); color: white; border-radius: 16px; padding: 24px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); border: 1px solid #e9ecef; transition: all 0.3s ease;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
            <div>
                <div class="stat-number" style="font-size: 36px; font-weight: 700; line-height: 1; margin-bottom: 4px;">{{ \App\Models\Document::count() }}</div>
                <div style="font-size: 14px; opacity: 0.9;">Total Arsip</div>
            </div>
            <a href="{{ url('/documents') }}" title="Lihat semua arsip" style="width: 48px; height: 48px; border-radius: 12px; background: rgba(255, 255, 255, 0.25); display: flex; align-items: center; justify-content: center; font-size: 20px; color: #fff; text-decoration:none; box-shadow: inset 0 0 0 1px rgba(255,255,255,0.25);">
    ...
    <div class="stat-card" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white; border-radius: 16px; padding: 24px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); border: 1px solid #e9ecef; transition: all 0.3s ease; position: relative;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
            <div>
                <div class="stat-number" style="font-size: 36px; font-weight: 700; line-height: 1; margin-bottom: 4px;">{{ \App\Models\Service::whereNull('parent_id')->count() }}</div>
        ...
        <div id="jenisDataDropdown" style="display:none; position:absolute; top:72px; right:24px; background:#fff; color:#2c3e50; border:1px solid #e9ecef; border-radius:12px; box-shadow:0 10px 24px rgba(0,0,0,0.15); min-width:240px; overflow:hidden; z-index:10;">
            ...
            <a href="{{ url('/documents') }}" style="display:block; padding:10px 14px; text-decoration:none; color:#2c3e50;">
                <i class="fas fa-list" style="margin-right:8px; color:#6c757d;"></i> Semua Jenis
            </a>
            @foreach(\App\Models\Service::whereNull('parent_id')->get(['id','name']) as $cat)
            <a href="{{ url('/documents?category_id='.$cat->id) }}" style="display:block; padding:10px 14px; text-decoration:none; color:#2c3e50;">
                <i class="fas fa-folder" style="margin-right:8px; color:#6c757d;"></i> {{ $cat->name }}
            </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="stat-card" style="background: linear-gradient(135deg, #17a2b8 0%, #6f42c1 100%); color: white; border-radius: 16px; padding: 24px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); border: 1px solid #e9ecef; transition: all 0.3s ease;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
            <div>
                <div class="stat-number" style="font-size: 36px; font-weight: 700; line-height: 1; margin-bottom: 4px;">{{ \App\Models\Document::whereDate('created_at', today())->count() }}</div>
                ...
                <div id="filterDropdown" style="display:none; position:absolute; top:110%; right:0; width:340px; background:#fff; border:1px solid #e9ecef; border-radius:14px; box-shadow:0 12px 28px rgba(0,0,0,0.12); padding:14px; z-index:20;">
                    ...
                    <div style="margin-bottom:10px;">
                        ...
                        <div style="display:grid; grid-template-columns: 1fr; gap:8px;">
                            @php $fixedCats = ['Permohonan Magang','Arsip','Dinas Kesehatan','Dinas Perhubungan']; @endphp
                            @foreach($fixedCats as $cname)
                            <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                <input type="radio" name="f_category" value="{{ \Illuminate\Support\Str::slug($cname,'_') }}" style="accent-color:#365F8D;">
                                <span style="color:#2c3e50; font-size:14px;">{{ $cname }}</span>
                            </label>
                            @endforeach
                            <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
    ...
    <div style="display:flex; gap:24px; align-items:flex-start; flex-wrap:wrap;">
        ...
        <div style="flex:1 1 360px; max-height:360px; overflow:auto; border:1px solid #e9ecef; border-radius:12px;">
            <table style="width:100%; border-collapse: collapse; font-size:14px;">
                ...
                <tbody>
                    @forelse($todayDocs->take(20) as $i => $doc)
                    <tr>
                        <td style="padding:10px 12px; border-bottom:1px solid #f3f4f6; color:#6c757d;">{{ $i+1 }}</td>
                        <td style="padding:10px 12px; border-bottom:1px solid #f3f4f6;">
                            <a href="#" style="color:#2c3e50; font-weight:600; text-decoration:none;">{{ $doc->title }}</a>
                        </td>
                        <td style="padding:10px 12px; border-bottom:1px solid #f3f4f6;">
                            <a href="#" style="color:#2c3e50; text-decoration:none;">{{ optional($doc->created_at)->format('d M Y H:i') }}</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
...
<script>
    ...
    const todayLabels = @json($labelsToday);
    const todayCounts = @json($countsToday);
